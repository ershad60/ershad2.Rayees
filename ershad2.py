...
 Telegram    : https://t.me/sultani1122
 Blogspot    : https://Ershadfiroozi.blogspot.com
--------------------------------------------------"""
back = 0
threads = []
successful = []
...
if "access_token" in q:
	...
	p.map(main, id)
	print "\033[1;97m----------------------------------------------"
	hamza('[âœ“] Process Has Been Completed.')
	hamza('\033[1;97m[âœ“] Checkpoint IDS Has Been Saved.')
	xx = str(len(oks))
	xxx = str(len(checkpoint))
	print ("[âœ“] Total \033[1;32mOK/\033[1;97mCP : \033[1;32m"+str(len(oks))+"/\033[1;97m"+str(len(checkpoint)))
	print (47*"-")
	raw_input("\nPress Enter To Back ")
	choice1()
...
def choice2_menu():
	...
	if c2m =="":
		...
		print (banner)
		try:
			idlist = raw_input("[âœ“] Enter File Path : ")
			pass1=raw_input("[1] Input Password  : ")
			pass2=raw_input("[2] Input Password  : ")
			print(47*"-")
			for line in open(idlist,"r").readlines():
		...
		choice2_menu()
	rana = str(len(id))
	hamza ("[âœ“] Total Friends: "+rana)
	time.sleep(0.5)
	hamza ("[âœ“] The Process Has Been Started.")
	time.sleep(0.5)
	hamza ("[!] Press CTRL Z To Stop Process.")
	time.sleep(0.5)
	print (47*"-")
